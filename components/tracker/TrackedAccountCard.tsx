...
import { TrackedAccount, SocialPlatform, TrackedPostItem, ToastMessage, TrackedPostMetrics } from '../../types';
import { PencilIcon, TrashIcon, PlusCircleIcon, SparklesIcon, ArrowUpIcon, ArrowDownIcon } from '../common/Icons';
...
const TrackedPostItemCard: React.FC<TrackedPostItemCardProps> = ({ post, platform, onEdit, onDelete, darkMode, addToast }) => {
  ...
  const handleGeminiAnalyze = async () => {
    if (!isGeminiAvailable() || !platform || !post.captionSummary) {
      const errorMsg = "Platform and caption summary are needed for AI analysis.";
    ...
    if (analysis) {
      ...
      const errorMsg = "Failed to get analysis from AI. Ensure API key is set and try again.";
      setAnalysisError(errorMsg);
      addToast(errorMsg, 'error');
    }
  };
  
  const cardBg = darkMode ? 'bg-slate-700 dark:border-slate-600' : 'bg-white border-gray-200'; // Inner post card
  const textColor = darkMode ? 'text-slate-300' : 'text-gray-500';
  const valueColor = darkMode ? 'text-slate-100' : 'text-gray-700';
  const linkColor = darkMode ? 'text-accent-highlight-dark hover:text-opacity-80' : 'text-accent-highlight-default hover:opacity-80';
  const aiButtonBg = darkMode ? 'bg-accent-700 hover:bg-accent-600 disabled:bg-slate-500 text-white' : 'bg-element-default hover:bg-gray-100 disabled:bg-gray-200 text-accent-highlight-default';
  ...
  return (
    <div className={`p-3 rounded-md border ${cardBg} shadow-sm`}>
      <div className="flex justify-between items-start mb-2">
        ...
        >
          Post: {post.postLinkOrIdentifier.length > 30 ? post.postLinkOrIdentifier.substring(0,27) + "..." : post.postLinkOrIdentifier}
        </a>
        <div className="flex space-x-1.5">
          <button onClick={onEdit} className={`p-1 rounded ${darkMode ? 'hover:bg-slate-600' : 'hover:bg-gray-100'} text-blue-500`} title="Edit Post Metrics">
            <PencilIcon className="w-4 h-4" />
          </button>
          <button onClick={onDelete} className={`p-1 rounded ${darkMode ? 'hover:bg-slate-600' : 'hover:bg-gray-100'} text-red-500`} title="Delete Post">
      ...
      {isGeminiAvailable() && post.captionSummary && (
        <div className="mt-2">
          <button onClick={handleGeminiAnalyze} disabled={isAnalyzing} className={`w-full text-xs px-2 py-1 rounded-md flex items-center justify-center ${aiButtonBg} transition`}>
...
type SortKey = keyof TrackedPostMetrics | 'postLinkOrIdentifier' | 'captionSummary';
...
export const TrackedAccountCard: React.FC<TrackedAccountCardProps> = ({
  ...
  const filteredAndSortedPosts = useMemo(() => {
    ...
    if (searchTerm) {
      posts = posts.filter(post =>
        post.postLinkOrIdentifier.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    posts.sort((a, b) => {
      ...
      if (sortKey === 'postLinkOrIdentifier' || sortKey === 'captionSummary') {
        ...
        valA = (sortKey === 'lastUpdated') ? new Date(a.metrics.lastUpdated).getTime() : a.metrics[sortKey as keyof TrackedPostMetrics] ?? 0;
        valB = (sortKey === 'lastUpdated') ? new Date(b.metrics.lastUpdated).getTime() : b.metrics[sortKey as keyof TrackedPostMetrics] ?? 0;
      }
  ...
  const SortButton: React.FC<{ columnKey: SortKey; label: string }> = ({ columnKey, label }) => (
    <button
    ...
    >
      {label}
      {sortKey === columnKey && (
        sortOrder === 'asc' ? <ArrowUpIcon className="w-3 h-3 ml-1" /> : <ArrowDownIcon className="w-3 h-3 ml-1" />
      )}
    </button>
  );
    ...
    <div className={mainCardClasses}>
      <div className={headerClasses}>
        <div className="flex items-center">
          ...
          <div>
            <div className="flex items-center">
              {platform?.icon && React.cloneElement(platform.icon, { className: `w-5 h-5 mr-1.5 ${platformIconColor}` })}
              <h3 className={`text-lg font-semibold ${usernameColor}`}>{account.usernameOrProfileLink}</h3>
            </div>
            <p className={`text-sm ${platformNameColor}`}>{platform?.name || 'Unknown Platform'}</p>
          </div>
        </div>
        <div className="flex items-center space-x-1 sm:space-x-2">
          <button
            onClick={() => onAddPost(account)}
            className={addPostButtonClasses} title="Add Post to Track"
          >
            <PlusCircleIcon className="w-4 h-4 sm:mr-1" /> <span className="hidden sm:inline">Add Post</span>
          </button>
          <button onClick={() => onEditAccount(account)} className={`p-1.5 rounded hover:bg-element-default dark:hover:bg-element-dark text-blue-500`} title="Edit Account">
            <PencilIcon className="w-4 h-4" />
          </button>
          <button onClick={() => onDeleteAccount(account.id)} className={`p-1.5 rounded hover:bg-element-default dark:hover:bg-element-dark text-red-500`} title="Delete Account">
            <TrashIcon className="w-4 h-4" />
          </button>
        </div>
      </div>
      ...
      <div className="p-3 sm:p-4">
        ...
        {account.posts.length > 0 && (
            <div className={`mb-3 p-2 rounded-md flex flex-wrap gap-2 items-center text-xs bg-element-default dark:bg-element-dark`}>
                <span className={`font-medium mr-1 text-muted-default dark:text-muted-dark`}>Sort by:</span>
        ...
        {filteredAndSortedPosts.length === 0 ? (
          <p className={`text-center py-4 text-muted-default dark:text-muted-dark`}>{account.posts.length > 0 ? 'No posts match your search.' : 'No posts tracked for this account yet.'}</p>
        ) : (
