...
interface TrackedItemFormProps {
  ...
  onSaveAccount: (
    accountData: Pick<
      TrackedAccount,
      "platformId" | "usernameOrProfileLink" | "id"
    >
  ) => void;
...
export const TrackedItemForm: React.FC<TrackedItemFormProps> = ({
  ...
  const [platformId, setPlatformId] = useState<string>(
    initialPost?.platformId ||
      targetAccount?.platformId ||
      (platforms.length > 0 ? platforms[0].id : "")
  );
  const [usernameOrProfileLink, setUsernameOrProfileLink] = useState<string>(
    targetAccount?.usernameOrProfileLink || ""
  );

  // Post fields
  const [postLinkOrIdentifier, setPostLinkOrIdentifier] = useState<string>(
    initialPost?.postLinkOrIdentifier || ""
  );
  const [captionSummary, setCaptionSummary] = useState<string>(
    initialPost?.captionSummary || ""
  );
  ...
  const [formMode, setFormMode] = useState<"account" | "post">(
    initialPost || accountForPost ? "post" : "account"
  );
  ...
  useEffect(() => {
    if (initialPost) {
      ...
      if (targetAccount)
        setUsernameOrProfileLink(targetAccount.usernameOrProfileLink); // Keep account context
    ...
    } else if (accountForPost) {
      // Adding new post to existing account
      setFormMode("post");
  ...
  const handleMetricChange = (
    field: keyof Omit<TrackedPostMetrics, "lastUpdated">,
    value: string
  ) => {
    setMetrics((prev) => ({
      ...prev,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (formMode === "account") {
      if (!platformId || !usernameOrProfileLink) {
        alert(
          "Platform and Username/Profile Link are required for tracking an account."
        );
        return;
      }
      onSaveAccount({
        id:
          initialAccount?.id ||
          Date.now().toString() + Math.random().toString(36).substring(2, 9), // Use existing ID if editing
        platformId,
      ...
      if (!platformId || !postLinkOrIdentifier) {
        alert(
          "Platform and Post Link/Identifier are required for tracking a post."
        );
        return;
      }
  ...
  const handleGeminiAnalyze = useCallback(async () => {
    if (
      ...
      setAnalysisError(
        "Platform, caption summary, and post identifier are needed for analysis."
      );
      return;
    }
    setIsAnalyzing(true);
    ...
    if (analysis) {
      ...
      setAnalysisError(
        "Failed to get analysis from AI. Please check API key and try again."
      );
    }
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <>
          <div>
            ...
            <select
              ...
              required
              disabled={isPostOperation && !!accountForPost}
            >
              {platforms.map((p) => (
          ...
          <div>
            <label htmlFor="usernameOrProfileLink" className={labelClasses}>
              Account Username / Profile Link
            </label>
            <input
              type="text"
              id="usernameOrProfileLink"
              value={usernameOrProfileLink}
              onChange={(e) => setUsernameOrProfileLink(e.target.value)}
              className={inputClasses}
              required
      ...
      {!initialAccount && !initialPost && !accountForPost && (
        <div className="my-4">
          ...
          <div className="flex space-x-4 mt-1">
            <label className="flex items-center space-x-2">
              <input
                ...
                name="formMode"
                value="account"
                checked={formMode === "account"}
                onChange={() => setFormMode("account")}
                className={`focus:ring-focus-default h-4 w-4 text-accent-highlight-default dark:text-accent-highlight-dark border-input-default dark:border-input-dark`}
              />
              <span className={`text-base-default dark:text-base-dark`}>
            ...
            <label className="flex items-center space-x-2">
              <input
                ...
                name="formMode"
                value="post"
                checked={formMode === "post"}
                onChange={() => setFormMode("post")}
                className={`focus:ring-focus-default h-4 w-4 text-accent-highlight-default dark:text-accent-highlight-dark border-input-default dark:border-input-dark`}
              />
              <span className={`text-base-default dark:text-base-dark`}>
                A Specific Post
          ...
          {formMode === "post" && !targetAccount?.id && (
            <p className={`text-xs mt-1 text-yellow-500 dark:text-yellow-400`}>
              You'll first define the account, then the post details.
            </p>
          )}
        </div>
      )}
      ...
      {(formMode === "post" || isEditingPost) && (
        <>
          <hr
            className={`border-element-default dark:border-element-dark my-4`}
          />
          <h3
            className={`text-lg font-medium text-accent-highlight-default dark:text-accent-highlight-dark`}
          >
            Post Details
          </h3>
          <div>
            <label htmlFor="postLinkOrIdentifier" className={labelClasses}>
              Post Link / Identifier
            </label>
            <input
              type="text"
              id="postLinkOrIdentifier"
              value={postLinkOrIdentifier}
              onChange={(e) => setPostLinkOrIdentifier(e.target.value)}
              className={inputClasses}
              required
              placeholder="e.g., URL to post or unique video ID"
            />
          </div>
          <div>
            <label htmlFor="captionSummary" className={labelClasses}>
              Caption Summary (for AI Analysis)
            </label>
            <textarea
              id="captionSummary"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label htmlFor="likes" className={labelClasses}>
                Likes
              </label>
              <input
                type="number"
          ...
          {isGeminiAvailable() && (postLinkOrIdentifier || captionSummary) && (
            <div
              className={`mt-4 p-3 rounded-md bg-element-default dark:bg-slate-700 border border-element-default dark:border-slate-600`}
            >
              <button
                type="button"
              ...
              {isAnalyzing && (
                <p
                  className={`text-xs mt-1 text-center text-muted-default dark:text-slate-400`}
                >
                  Analyzing...
                </p>
              )}
              ...
              {analysisResult && (
                <div
                  className={`mt-2 p-2 rounded text-sm bg-green-100 text-green-700 dark:bg-green-700 dark:text-muted-default`}
                >
      ...
      <div className="flex justify-end space-x-3 pt-2">
        ...
        <button type="submit" className={`${buttonBase} ${primaryButton}`}>
          {isEditingPost || (formMode === "post" && targetAccount?.id)
            ? "Save Post"
            : isEditingAccount
