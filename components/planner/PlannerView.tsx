...
export const PlannerView: React.FC<PlannerViewProps> = ({
  ...
  const [showDiscoverTrendsModal, setShowDiscoverTrendsModal] =
  ...
  const [selectedPlatformForTrends, setSelectedPlatformForTrends] =
    useState<string>(
  ...
  const handleUpdateStatus = useCallback(
    (postId: string, newStatus: ScheduledPost["status"]) => {
  ...
  // Unificarea funcției de salvare pentru a gestiona atât adăugarea, cât și editarea
  const handleSavePost = useCallback(
    (post: ScheduledPost | Omit<ScheduledPost, "id" | "status">) => {
      if ("id" in post && "status" in post) {
        // Este un ScheduledPost complet, deci este o actualizare
        handleUpdatePost(post as ScheduledPost);
      } else {
        // Este un Omit<ScheduledPost, "id" | "status">, deci este o adăugare
        handleAddPost(post);
      }
    },
    [handleAddPost, handleUpdatePost]
  );

  const fetchTrendingTopicsForModal = async () => {
    if (
      !selectedPlatformForTrends ||
      !nicheForTrends.trim() ||
      !isGeminiAvailable()
    ) {
    ...
    setTrendingTopics([]);
    const platformName =
      DEFAULT_PLATFORMS.find((p) => p.id === selectedPlatformForTrends)?.name ||
      "selected platform";
    ...
    if (trends) {
      ...
      addToast(
        `Workspaceed trends for ${platformName} / ${nicheForTrends}!`,
        "success"
      );
    } else {
      setTrendsError(
        "Failed to fetch trending topics. Please check API key and try again."
      );
  ...
  const handleDownloadPlannerCSV = () => {
    ...
    exportToCSV(dataToExport, "planner_schedule.csv", [
      "ID",
      "Platform",
      "Account",
      "Content",
  ...
  const filteredAndSortedPosts = useMemo(() => {
    return scheduledPosts
      .filter((post) => {
        const platformMatch =
          filterPlatform === "all" || post.platformId === filterPlatform;
        const statusMatch =
  ...
  const plannerStats = useMemo(() => {
    ...
    const currentlyScheduled = scheduledPosts.filter(
  ...
  const statLabelClass = `text-xs text-muted-secondary dark:text-muted-dark uppercase`;
  const statValueClass = `text-2xl text-accent-highlight-default font-bold text-button-accent-default dark:text-button-accent-dark`;

  return (
    <div className="bg-card-default2 dark:bg-page-dark text-base-default dark:text-base-dark p-4 rounded-lg shadow-lg">
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-3">
        <h2 className="text-2xl font-semibold flex items-center">
          <CalendarIcon
            className={`w-7 h-7 mr-2 text-accent-highlight-default dark:text-button-accent-dark`}
          />
          Content Planner
        </h2>
        {viewState === ViewState.LIST && (
          <div className="flex flex-wrap gap-2">
            ...
            <button onClick={openSchedulerForm} className={buttonClasses}>
      ...
      {viewState === ViewState.LIST && (
        <div
          className={`mb-6 p-4 rounded-md bg-element-default dark:bg-element-dark flex flex-col sm:flex-row gap-4 items-center`}
        >
          <div className="flex-grow w-full sm:w-auto">
            <label
              htmlFor="filterPlatform"
              className={`block text-sm font-medium mb-1 text-muted-secondary dark:text-muted-dark`}
            >
              Filter by Platform:
            </label>
            <select
      ...
      <Modal
        isOpen={showDiscoverTrendsModal}
        onClose={() => setShowDiscoverTrendsModal(false)}
        title="Discover Trends with AI"
        darkMode={darkMode}
      >
        <div className="space-y-3">
          <div>
            <label
              htmlFor="platformForTrends"
              className={`block text-sm font-medium text-muted-black dark:text-muted-dark mb-1`}
            >
              Platform:
            </label>
            <select
          ...
          >
            {trendsLoading ? (
              ...
              <>
                <FireIcon className="w-4 h-4 mr-2" /> Discover Trends
              </>
            )}
          </button>
          {trendsError && (
          ...
          {trendingTopics.length > 0 && (
            <div className="mt-4 max-h-60 overflow-y-auto space-y-2">
              <h4
                className={`text-md font-semibold text-base-default dark:text-base-dark`}
              >
                Trending Topics:
              </h4>
              {trendingTopics.map((trend, index) => (
                <div
                  key={index}
                  className={`p-2 rounded bg-element-default dark:bg-element-dark`}
                >
                  <p
                    className={`font-semibold text-button-accent-default dark:text-button-accent-dark`}
                  >
                    {trend.topic}
                  </p>
                  {trend.reason && (
                    <p
                      className={`text-xs text-muted-default dark:text-muted-dark`}
                    >
                      <em>Reason:</em> {trend.reason}
                    </p>
                  )}
                  {trend.contentIdea && (
                    <p
                      className={`text-xs text-base-default dark:text-base-dark`}
                    >
                      <em>Idea:</em> {trend.contentIdea}
                    </p>
                  )}
                </div>
              ))}
      ...
      {viewState === ViewState.LIST && (
        <>
          {filteredAndSortedPosts.length === 0 ? (
            <div
              className={`text-center py-10 text-muted-default dark:text-muted-dark`}
            >
              <CalendarIcon className="w-16 h-16 mx-auto mb-4 opacity-50" />
              <p className="text-xl">
                No posts match your filters, or none scheduled yet.
              </p>
              <p className="mt-1">
                Ready to plan your next big hit? Click "Schedule Post" to get
                started or adjust your filters!
              </p>
            </div>
          ) : (
